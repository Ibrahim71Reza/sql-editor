...
export function resultToCsv(fields, rows) {
  ...
  const body = rows.map((row) => fields.map((field) => csvEscape(row[field.name])).join(",")).join("\n");
...
export function buildImportSql({ tableName, headers, rows }) {
  ...
  return [`BEGIN;`, create, ...chunks, `COMMIT;`].join("\n\n");
}
...
export function inferChart(fields, rows) {
  ...
  const numericField = profiles.find((p) => p.numeric)?.name;
  const labelField = profiles.find((p) => !p.numeric && p.unique <= Math.min(rows.length, 30))?.name || fields[0]?.name;
  if (!numericField || !labelField || numericField === labelField) return null;
  const data = rows.slice(0, 25).map((row) => ({ label: String(row[labelField] ?? "NULL"), value: Number(row[numericField]) || 0 }));
